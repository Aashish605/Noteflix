...
const Course = () => {
    ...
    useEffect(() => {
        const getcourse = async () => {
            try {
                const res = await axios.get(`https://noteflix-backend.onrender.com/papers`);
    ...
    return (
        <>
            { 
                CourseData ? <div>
                                <div>
                <h1 className="text-center text-4xl mt-16">{CourseData.title} Papers </h1>
                    ...
                    {CourseData.data && CourseData.data.map((i) => (
                        <div key={i.sem} className="m-8 w-full">
                            ...
                            <div className="w-[80vw] flex flex-wrap gap-8 my-4 justify-center">
                                {i.subjects.map((e) => (
                                    <NavLink
                                        to={`/pastpapers/${i.sem}/${CourseData.title}/${e.name}/${e.link}`}
                                        key={e.name}
